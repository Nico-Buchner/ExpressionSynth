...
public:
        ...
        : state (s), paramID (std::move (id)), label (std::move (labelText))
    {
        ...
        // The panes live in Viewports, and on iOS a Viewport scrolls on
        // any touch drag by default - including one that starts on this
        // slider, so adjusting a value scrolls the pane instead. This
        // flag tells the Viewport to leave such drags to the slider;
        // drags on the background still scroll.
